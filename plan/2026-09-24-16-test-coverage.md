...
Closes #16, #24. Two independent groups land as separate commits in
`tests/cli.sh` (plus two stub files and one helper change for Group B), no
...
- **Group B** (after Group A): the #24 dispatch/usage-error branches, two
  ...
  boolean matching the existing `STUB_PROCESSES` family) plus a `mkproj`
  parameter for a symlinked `.devenv`.
...
count. This plan uses the recomputed 16 (final total 82, not 83) as the
target. **Treat the actual `cli: N passed, M failed` line as ground truth
over any number in this plan or the spec** — if a commit's real count
differs from the one given here, stop and diff against the expected new
`expect` call-sites before continuing, don't just edit the number and move
on.

## Steps

1. **`tests/cli.sh`, Group A — four removal refusals.** Insert after the
   existing `expect 2 "remove: dev mismatch" ...` line (currently line 288,
   ...
   prints `cli: 66 passed, 0 failed` (62 + 4). `node --test 'tests/model/*.test.js'`
   is untouched, still 7/7 files.
...
2. **`tests/stub/devenv` — add the failure hooks, no behaviour change yet.**
   In the `init)` arm (currently starts at line 7), insert as the very first
   line of the arm, before the `cat >devenv.nix` heredoc:
   ```sh
   init)
     if [ "${STUB_INIT_FAIL:-}" = 1 ]; then echo "stub devenv: init failed" >&2; exit 1; fi
     cat >devenv.nix <<'NIX'
   ```
   Split `revoke` out of the combined arm (currently line 48,
   ...
   `$STUB_LOG` write (line 5) stays first, ahead of both new checks, so a
   forced failure still logs an attempted call (needed by step 11's and
   step 9's assertions later — do not reorder this).
   Verify by: re-run the full suite with no new tests yet — must still print
   `cli: 66 passed, 0 failed` (unchanged from step 1; this step is additive
   and inert with both vars unset).
...
3. **`tests/stub/nix` — add `STUB_RUN_FAIL`, no behaviour change yet.** In
   ...
   *" run "*)
     ...
     printf '{ }\n' >devenv.nix
   ```
   Same rule: `$STUB_LOG` write (line 5) stays first.
   Verify by: still `cli: 66 passed, 0 failed`.
...
4. **`tests/cli.sh` — `mkproj` takes an optional second arg.** Replace the
   ...
   `mkproj d`) is byte-identical to before — same files, same `.devenv/state/db/x`,
   same `.devenv/profile`. No new call site yet.
   Verify by: still `cli: 66 passed, 0 failed` — confirms the refactor
...
5. **`tests/cli.sh` — dispatch and usage-error branches (spec tests 5–8).**
   ...
   ```
   Note the wording difference is deliberate: `list` says "unknown
   argument" (`pkgs/cli.sh:327`), the other three say "unknown option"
   (`pkgs/cli.sh:134`, `253`, `487`) — match each site's actual string.
   9 new `expect` calls (1 + 3 + 1 + 4).
   Verify by: `cli: 75 passed, 0 failed` (66 + 9).
...
6. **`tests/cli.sh` — personal template keeps an existing `devenv.yaml`
   ...
   "templates with personal" / skip-warning assertions, currently ending
   around line 81), or anywhere after `personal_dir` fixtures exist:
   ```sh
   d=$(fresh)
   ...
   section (has both `devenv.nix` and `devenv.yaml`, confirmed present in
   the existing fixture at lines 69–76). +1 `expect`.
   Verify by: `cli: 76 passed, 0 failed`.
...
7. **`tests/cli.sh` — personal template, valid JSON, wrong version (spec
   test 10).** Add as its own `templates --json` call, after the existing
   personal-templates block, so it doesn't disturb the counts/warnings that
...
8. **`tests/cli.sh` — generator path when `nix` is missing (spec test
   11).** Add in the init section, near the other generator tests:
   ```sh
   mkdir -p "$root/nonix"
   ...
   ```
   `with_no_nix` is built by hand (only `devenv` symlinked in) rather than
   trimming `with_devenv`'s PATH at call time — `base_path` (line 28) only
   ever holds the fixed tool list (line 25), which has no `nix` or `devenv`
   in it, so this is nix-free by construction, not by omission. This does
   **not** touch or loosen the existing `base_path`/`with_devenv` PATH
   guarantee — it only adds a third, narrower PATH variant alongside them.
   +1 `expect`.
   Verify by: `cli: 78 passed, 0 failed`.
   Commit: `test(cli): the generator path when nix is missing (#24, step 8)`.

9. **`tests/cli.sh` — `devenv init` failing (spec test 12).** Add near the
   other init tests, using the `STUB_INIT_FAIL` hook from step 2:
   ```sh
   d=$(fresh)
...
10. **`tests/cli.sh` — generator flake run failing (spec test 13).** Add
    near the other generator tests, using `STUB_RUN_FAIL` from step 3:
    ```sh
    d=$(fresh)
...
11. **`tests/cli.sh` — `devenv revoke` failing, removal still proceeds
    ...
    successful-removal tests, using `STUB_REVOKE_FAIL` from step 2. Uses
    `mkproj f` (a fresh, ordinary project, `dir` kind — default):
    ```sh
    p=$(mkproj f)
    ...
    ```
    `STUB_PROCESSES=stopped` is set explicitly here rather than relied on as
    `rm_cli`'s default (`tests/cli.sh:271` on `main` already defaults it to
    `stopped`, but spelling it out keeps this test correct regardless of
    which base it lands on or how `rm_cli` changes later). Do not call this
    through `rm_cli` if that would double up `STUB_PROCESSES`/`env` — call
    `with_devenv env ... "$cli" remove ...` directly as shown, matching the
    spec. +1 `expect`.
    Verify by: `cli: 81 passed, 0 failed`.
    Commit: `test(cli): a failed devenv revoke warns but does not block removal (#24, step 11)`.

12. **`tests/cli.sh` — `.devenv` is a symlink (spec test 15).** Add in the
    remove section, using the `mkproj ... symlink` variant from step 4:
    ```sh
    p=$(mkproj e symlink)
...
13. **`tests/cli.sh` — extend the existing "status fail" assertion (spec
    ...
    existing `expect 0 "status fail" ...` block (currently around line
    252–253):
    ```sh
    jq -e '.detail | test("boom")' "$root/out" >/dev/null || bad "status: fail detail names the stderr"
    ```
    +0 `expect` calls (extends an existing one).
    Verify by: still `cli: 82 passed, 0 failed` (same call count, one more
...
```
Expected `node` result throughout: 7/7 files pass (no `Model.js` changes anywhere
in this plan).
...
| --- | --- | --- |
| baseline | — | 62 |
| 1 | Group A (4 refusals) | 66 |
| 2 | stub devenv hooks (inert) | 66 |
| 3 | stub nix hook (inert) | 66 |
| 4 | mkproj param (inert) | 66 |
| 5 | dispatch/usage-error (9) | 75 |
| 6 | personal keeps devenv.yaml | 76 |
| 7 | wrong-version personal | 77 |
| 8 | generator without nix | 78 |
| 9 | devenv init fails | 79 |
| 10 | generator run fails | 80 |
| 11 | revoke fails | 81 |
| 12 | .devenv symlink | 82 |
| 13 | status detail (no new `expect`) | 82 |

**Final: `cli: 82 passed, 0 failed`** (62 baseline + 4 Group A + 16 Group B).
...
from today's stub. Reverting steps 2–4 alone (without reverting steps 9–12,
which use them) would break the build — always revert the consuming test
step first, or revert the whole stack from the top down.
...
- Every removal test in this plan (steps 1, 11, 12) points only at
  ...
  (itself `$root/home`, never the invoking user's `$HOME` — `tests/cli.sh:15`
  exports it) — never at a real project. Test 4 in step 1 (`remove: target
  is /`) is the one case that names `/` directly as the CLI's target; it is
  safe only because `pkgs/cli.sh:500`'s `[ "$dir" != / ]` check runs before
  every other check in `cmd_remove` — the HOME check, the root check, the
  `devenv.nix` check, `cmd_status`, and `devenv revoke` — so nothing mutating
  runs first. This ordering is load-bearing for the test's safety; if a
...
- The `STUB_LOG` write in both stubs must stay the first line, ahead of the
  new `STUB_INIT_FAIL`/`STUB_REVOKE_FAIL`/`STUB_RUN_FAIL` gates (steps 2–3
  keep it that way) — any future edit to either stub that moves the log
  write after the gate silently breaks the "a forced failure still shows up
  in the log as an attempted call" property steps 9, 10 and 11 rely on.
